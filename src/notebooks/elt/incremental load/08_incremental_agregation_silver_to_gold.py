...
logging.info("Starting incrmental agregation from MinIO silver to MinIO gold...")
...
for table_name in config_file.queries_gold.keys():
    ...
    try:
        ...
        df_max_modifieddate_gold = spark.read.format("delta").load(gold_table_path) \
        ...
        if rows_to_update == 0:
            ...
            df_with_update_date = func_file.add_data_last_update(query_update_data_from_gold)
            ...
            df_with_month_partition.write.format("delta").mode("append").partitionBy("month_key").save(gold_table_path)
            
            # Logging the sucessfully process
            logging.info(f"Table {table_name} Sucessfully updated and saved in MinIO silver on: {gold_table_path}")
